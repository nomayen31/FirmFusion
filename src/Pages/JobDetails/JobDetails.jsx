...
const JobDetails = () => {
  ...
  useEffect(() => {
    // NOTE: This logic assumes 'jobId' is unique across all jobs in 'companies.json'
    fetch("/public/companies.json")
      .then((res) => res.json())
      .then((companies) => {
        let selectedJob = null;
        for (const company of companies) {
          // Ensure comparison is type-safe if IDs are different types (e.g., string vs number)
          selectedJob = company.jobs?.find((job) => job.id.toString() === jobId.toString());
          if (selectedJob) {
            // Also attach the company name and logo to the job object for context
  ...
  // Helper function for the "Apply Now" action
  const handleApply = () => {
    // In a real application, this would redirect to an application form or external link
    ...
    <div className="min-h-screen px-4 py-16  bg-[#0A0C34] sm:px-6 lg:px-8">
      <div className="mx-auto mt-12 max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-3">
          ...
          {/* Column 1: Job Description (Takes 2/3 width) */}
          ...
          {/* Column 2: Sticky Sidebar (Takes 1/3 width) */}
              ...
              <div className="p-6 bg-white border border-gray-100 shadow-lg rounded-xl">
                ...
                <ul className="space-y-3">
                  <li className="flex items-center justify-between text-gray-700">
                    <span className="flex items-center font-medium"><FiBriefcase className="mr-2 text-indigo-500" /> Job Type</span>
                    <span className="font-semibold text-gray-900">{job.jobType || "N/A"}</span>
                  </li>
                  <li className="flex items-center justify-between text-gray-700">
                    <span className="flex items-center font-medium"><FiDollarSign className="mr-2 text-indigo-500" /> Salary Range</span>
                    <span className="font-semibold text-gray-900">{job.salary || "Negotiable"}</span>
                  </li>
                  <li className="flex items-center justify-between text-gray-700">
                    <span className="flex items-center font-medium"><FiMapPin className="mr-2 text-indigo-500" /> Location</span>
                    <span className="font-semibold text-gray-900">{job.location || "Remote"}</span>
                  </li>
                  {/* Additional details can be added here (e.g., posted date, experience level) */}
                </ul>
              </div>
