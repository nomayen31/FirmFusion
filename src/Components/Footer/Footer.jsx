import React from "react";

export default function Footer() {
  return (
    <footer className="relative isolate bg-slate-950 text-slate-300">
      {/* gradient aura */}
      <div className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(80rem_40rem_at_50%_-10%,rgba(59,130,246,0.15),transparent)]" />

      <div className="w-full px-6 py-16 mx-auto max-w-7xl sm:py-20 lg:px-8">
        <div className="md:flex md:items-start md:justify-between">
          {/* Brand */}
          <div className="max-w-sm mb-10 md:mb-0">
            <a href="#" className="flex items-center gap-3">
              {/* Logo */}
              <span className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-500 to-blue-500 ring-1 ring-white/10">
                <svg
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                  className="w-6 h-6 text-white"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                >
                  <path d="M4 7h16M4 12h16M4 17h16" strokeLinecap="round" />
                </svg>
              </span>
              <span className="text-2xl font-semibold tracking-tight text-white">FirmFusion </span>
            </a>
            <p className="mt-4 text-sm leading-6 text-slate-400">
              Beautifully crafted Tailwind components and templates to ship apps faster. Open‑source and community‑driven.
            </p>

            {/* Socials */}
            <div className="flex gap-3 mt-6">
              {[
                {
                  label: "Facebook",
                  href: "#",
                  icon: (
                    <svg className="w-4 h-4" viewBox="0 0 8 19" fill="currentColor" aria-hidden>
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M6.135 3H8V0H6.135A4.147 4.147 0 0 0 1.993 4.142V6H0v3h2v9.938h3V9h2.021l.592-3H5V3.591A.6.6 0 0 1 5.592 3h.543Z"
                      />
                    </svg>
                  ),
                },
                {
                  label: "Discord",
                  href: "#",
                  icon: (
                    <svg className="w-4 h-4" viewBox="0 0 21 16" fill="currentColor" aria-hidden>
                      <path d="M16.942 1.556a16.3 16.3 0 0 0-4.126-1.3 12.04 12.04 0 0 0-.529 1.1 15.175 15.175 0 0 0-4.573 0 11.585 11.585 0 0 0-.535-1.1 16.274 16.274 0 0 0-4.129 1.3A17.392 17.392 0 0 0 .182 13.218a15.785 15.785 0 0 0 4.963 2.521c.41-.564.773-1.16 1.084-1.785a10.63 10.63 0 0 1-1.706-.83c.143-.106.283-.217.418-.33a11.664 11.664 0 0 0 10.118 0c.137.113.277.224.418.33-.544.328-1.116.606-1.71.832a12.52 12.52 0 0 0 1.084 1.785 16.46 16.46 0 0 0 5.064-2.595 17.286 17.286 0 0 0-2.973-11.59ZM6.678 10.813a1.941 1.941 0 0 1-1.8-2.045 1.93 1.93 0 0 1 1.8-2.047 1.919 1.919 0 0 1 1.8 2.047 1.93 1.93 0 0 1-1.8 2.045Zm6.644 0a1.94 1.94 0 0 1-1.8-2.045 1.93 1.93 0 0 1 1.8-2.047 1.918 1.918 0 0 1 1.8 2.047 1.93 1.93 0 0 1-1.8 2.045Z" />
                    </svg>
                  ),
                },
                {
                  label: "Twitter",
                  href: "#",
                  icon: (
                    <svg className="w-4 h-4" viewBox="0 0 20 17" fill="currentColor" aria-hidden>
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M20 1.892a8.178 8.178 0 0 1-2.355.635 4.074 4.074 0 0 0 1.8-2.235 8.344 8.344 0 0 1-2.605.98A4.13 4.13 0 0 0 13.85 0a4.068 4.068 0 0 0-4.1 4.038 4 4 0 0 0 .105.919A11.705 11.705 0 0 1 1.4.734a4.006 4.006 0 0 0 1.268 5.392 4.165 4.165 0 0 1-1.859-.5v.05A4.057 4.057 0 0 0 4.1 9.635a4.19 4.19 0 0 1-1.856.07 4.108 4.108 0 0 0 3.831 2.807A8.36 8.36 0 0 1 0 14.184 11.732 11.732 0 0 0 6.291 16 11.502 11.502 0 0 0 17.964 4.5c0-.177 0-.35-.012-.523A8.143 8.143 0 0 0 20 1.892Z"
                      />
                    </svg>
                  ),
                },
                {
                  label: "GitHub",
                  href: "#",
                  icon: (
                    <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden>
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M10 .333A9.911 9.911 0 0 0 6.866 19.65c.5.092.678-.215.678-.477 0-.237-.01-1.017-.014-1.845-2.757.6-3.338-1.169-3.338-1.169a2.627 2.627 0 0 0-1.1-1.451c-.9-.615.07-.6.07-.6a2.084 2.084 0 0 1 1.518 1.021 2.11 2.11 0 0 0 2.884.823c.044-.503.268-.973.63-1.325-2.2-.25-4.516-1.1-4.516-4.9A3.832 3.832 0 0 1 4.7 7.068a3.56 3.56 0 0 1 .095-2.623s.832-.266 2.726 1.016a9.409 9.409 0 0 1 4.962 0c1.89-1.282 2.717-1.016 2.717-1.016.366.83.402 1.768.1 2.623a3.827 3.827 0 0 1 1.02 2.659c0 3.807-2.319 4.644-4.525 4.889a2.366 2.366 0 0 1 .673 1.834c0 1.326-.012 2.394-.012 2.72 0 .263.18.572.681.475A9.911 9.911 0 0 0 10 .333Z"
                      />
                    </svg>
                  ),
                },
                {
                  label: "Dribbble",
                  href: "#",
                  icon: (
                    <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden>
                      <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M10 0a10 10 0 1 0 10 10A10.009 10.009 0 0 0 10 0Zm6.613 4.614a8.523 8.523 0 0 1 1.93 5.32 20.094 20.094 0 0 0-5.949-.274c-.059-.149-.122-.292-.184-.441a23.879 23.879 0 0 0-.566-1.239 11.41 11.41 0 0 0 4.769-3.366ZM8 1.707a8.821 8.821 0 0 1 2-.238 8.5 8.5 0 0 1 5.664 2.152 9.608 9.608 0 0 1-4.476 3.087A45.758 45.758 0 0 0 8 1.707ZM1.642 8.262a8.57 8.57 0 0 1 4.73-5.981A53.998 53.998 0 0 1 9.54 7.222a32.078 32.078 0 0 1-7.9 1.04h.002Zm2.01 7.46a8.51 8.51 0 0 1-2.2-5.707v-.262a31.64 31.64 0 0 0 8.777-1.219c.243.477.477.964.692 1.449-.114.032-.227.067-.336.1a13.569 13.569 0 0 0-6.942 5.636l.009.003ZM10 18.556a8.508 8.508 0 0 1-5.243-1.8 11.717 11.717 0 0 1 6.7-5.332.509.509 0 0 1 .055-.02 35.65 35.65 0 0 1 1.819 6.476 8.476 8.476 0 0 1-3.331.676Zm4.772-1.462A37.232 37.232 0 0 0 13.113 11a12.513 12.513 0 0 1 5.321.364 8.56 8.56 0 0 1-3.66 5.73h-.002Z"
                      />
                    </svg>
                  ),
                },
              ].map((s) => (
                <a
                  key={s.label}
                  href={s.href}
                  aria-label={s.label}
                  className="inline-flex items-center justify-center transition border rounded-full h-9 w-9 border-white/10 bg-white/5 text-slate-300 hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                >
                  {s.icon}
                </a>
              ))}
            </div>
          </div>

          {/* Links + Newsletter */}
          <div className="grid flex-1 grid-cols-2 gap-8 sm:grid-cols-3 lg:grid-cols-4">
            <nav className="space-y-4">
              <h3 className="text-xs font-semibold tracking-wide uppercase text-slate-400">Resources</h3>
              <ul className="space-y-3 text-sm">
                <li><a className="hover:text-white" href="#">Flowbite</a></li>
                <li><a className="hover:text-white" href="#">Tailwind CSS</a></li>
                <li><a className="hover:text-white" href="#">Docs</a></li>
                <li><a className="hover:text-white" href="#">Components</a></li>
              </ul>
            </nav>

            <nav className="space-y-4">
              <h3 className="text-xs font-semibold tracking-wide uppercase text-slate-400">Follow us</h3>
              <ul className="space-y-3 text-sm">
                <li><a className="hover:text-white" href="#">GitHub</a></li>
                <li><a className="hover:text-white" href="#">Discord</a></li>
                <li><a className="hover:text-white" href="#">Twitter / X</a></li>
                <li><a className="hover:text-white" href="#">YouTube</a></li>
              </ul>
            </nav>

            <nav className="space-y-4">
              <h3 className="text-xs font-semibold tracking-wide uppercase text-slate-400">Legal</h3>
              <ul className="space-y-3 text-sm">
                <li><a className="hover:text-white" href="#">Privacy Policy</a></li>
                <li><a className="hover:text-white" href="#">Terms & Conditions</a></li>
                <li><a className="hover:text-white" href="#">Cookie Policy</a></li>
                <li><a className="hover:text-white" href="#">Licenses</a></li>
              </ul>
            </nav>

            {/* Newsletter */}
            <div className="max-w-sm">
              <h3 className="text-xs font-semibold tracking-wide uppercase text-slate-400">Stay in the loop</h3>
              <p className="mt-4 text-sm text-slate-400">Get updates on new components, templates, and tips.</p>
              <form
                onSubmit={(e) => e.preventDefault()}
                className="flex gap-2 mt-4"
              >
                <input
                  type="email"
                  required
                  placeholder="Your email"
                  className="w-full px-3 py-2 text-sm text-white rounded-xl bg-white/5 placeholder:text-slate-500 ring-1 ring-inset ring-white/10 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                />
                <button
                  type="submit"
                  className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white transition shadow-sm whitespace-nowrap rounded-xl bg-gradient-to-tr from-indigo-500 to-blue-500 ring-1 ring-white/10 hover:brightness-110 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                >
                  Subscribe
                </button>
              </form>
            </div>
          </div>
        </div>

        <hr className="my-12 border-white/10" />

        {/* bottom bar */}
        <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
          <p className="text-xs text-slate-400">
            © {new Date().getFullYear()} <a className="hover:text-white" href="#">Flowbite™</a>. All rights reserved.
          </p>
          <div className="flex items-center gap-4 text-xs text-slate-400">
            <a className="hover:text-white" href="#">Status</a>
            <span className="w-px h-3 bg-white/10" />
            <a className="hover:text-white" href="#">Changelog</a>
            <span className="w-px h-3 bg-white/10" />
            <a className="hover:text-white" href="#">Contact</a>
          </div>
        </div>
      </div>
    </footer>
  );
}
